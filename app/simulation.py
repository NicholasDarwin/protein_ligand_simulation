...
def run_gromacs_simulation(protein_name, ligand_name):
    ...
    subprocess.run(["gmx", "solvate", "-cp", f"data/{ligand_name}_box.gro", "-cs", "spc216.gro", "-o", f"data/{ligand_name}_solv.gro"])
    subprocess.run(["gmx", "grompp", "-f", "gromacs/minim.mdp", "-c", f"data/{ligand_name}_solv.gro", "-p", "data/topol.top", "-o", f"data/{ligand_name}_minim.tpr"])
    subprocess.run(["gmx", "mdrun", "-v", "-deffnm", f"data/{ligand_name}_minim"])
    '''